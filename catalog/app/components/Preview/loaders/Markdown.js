...
const signImg = ({ signer, handle }) => R.evolve({
  src: (src) =>
    signer.signResource({
...
const processLink = ({ urls, signer, handle }) => R.evolve({
  href: R.pipe(
    Resource.parse,
...
const fetch = utils.gatedS3Request(utils.objectGetter(
  (r, { handle, signer, urls }) => {
...
export const load = (handle, callback) =>
  utils.withSigner((signer) =>
    utils.withRoutes(({ urls }) =>
      fetch(handle, callback, { signer, urls })));
